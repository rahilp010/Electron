...
const Sidebar = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div
        className={`bg-white shadow-lg transition-all duration-300 ease-in-out w-56 flex flex-col`}
      >
        {/* Logo Section */}
        <div className="p-4">
              ...
              {mainMenuItems.map((item, index) => (
                <div key={index}>
                  <div
                    ...
                    onClick={() => item.hasSubmenu && toggleMenu(item.submenuKey)}
                  >
                    ...
                    {item.hasSubmenu && (
                      <ChevronDown
                        className={`w-4 h-4 transition-transform duration-200 ${
                          expandedMenus[item.submenuKey] ? 'rotate-180' : ''
                        }`}
                      />
                    )}
                  </div>
                  ...
                  {/* Submenu */}
                  {item.hasSubmenu && item.submenu && expandedMenus[item.submenuKey] && (
