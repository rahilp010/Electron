...
export const electronSlice = createSlice({
    ...
    reducers: {
        ...
        updateTransaction: (state, action) => {
            ...
            const index = state.transaction.data.findIndex(transaction => transaction.id === updatedTransaction.id);
            if (index !== -1) {
        ...
        deleteTransaction: (state, action) => {
            ...
            state.transaction.data = state.transaction.data.filter(transaction => transaction.id !== action.payload);
            localStorage.setItem('transaction', JSON.stringify(state.transaction.data));
        },
            ...
            try {
                ...
                if (data) {
                    ...
                    if (Array.isArray(parsed)) {
                        ...
                        console.warn('transaction data in localStorage is not an array, resetting to empty array');
                        state.transaction.data = [];
...
export const { createProduct, updateProduct, deleteProduct, setProducts, getProducts, createClient, updateClient, deleteClient, setClients, getClients, addTransaction, updateTransaction, deleteTransaction, setTransactions, getTransaction } = electronSlice.actions;
