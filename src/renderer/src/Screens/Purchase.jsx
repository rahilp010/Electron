...
const usePurchaseOperations = () => {
  ...
  const handleDeleteGroup = useCallback(
    async (items, billNo) => {
      if (
        !window.confirm(
          `Are you sure you want to delete ${items.length} purchases in bill ${billNo}?`
        )
      )
        return
  ...
  const handleEditGroup = useCallback(
    (items, setSelectedTransaction, setIsUpdateExpense, setShowPurchaseBillModal) => {
      const groupData = {
        ...items[0],
...
const Purchase = () => {
    ...
    const sortedBillNos = Object.keys(grouped).sort((a, b) => {
      ...
      const getVal = (groupItems, item, key) => {
        switch (key) {
          ...
          case 'totalAmount':
            return groupItems.reduce((sum, i) => sum + (i.totalAmountWithTax || 0), 0)
          case 'pendingAmount':
            return groupItems.reduce((sum, i) => {
              if (i.statusOfTransaction === 'pending') {
                return (
                  sum +
                  (i.paymentType === 'partial' ? i.pendingAmount || 0 : i.totalAmountWithTax || 0)
                )
              }
              return sum
            }, 0)
  ...
  }, [filteredTransactions, sortConfig, clients, products])
                ...
                <tbody className="text-sm divide-y divide-gray-200">
                  {Object.entries(filteredGrouped).length === 0 ? (
                    ...
                    renderedGroups.map(([billNo, items]) => {
                      if (items.length === 1) {
                        // Single item: Render directly as PurchaseRow without group header
                        const item = items[0]
                        return (
                          <PurchaseRow
