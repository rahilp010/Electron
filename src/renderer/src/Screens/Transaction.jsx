...
const Transaction = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    let result = transaction.filter((data) => {
      ...
      const matchesProduct = productFilter ? String(data.productId) === String(productFilter) : true
  ...
  const handlePendingAmount = () => {
    ...
    const fullPayment = transaction
      .filter((item) => item.statusOfTransaction === 'pending' && item.paymentType === 'full')
      .reduce((total, item) => total + item.sellAmount * item.quantity, 0)

    const paritalPayment = transaction
      .filter((item) => item.statusOfTransaction === 'pending' && item.paymentType === 'partial')
        ...
        <div className="border border-gray-200 shadow px-5 py-3 mx-6 rounded-3xl my-4 flex">
          <div className="mx-5 border-r w-52">
            ...
            <p className="text-2xl font-light">
              ₹{' '}
              {toThousands(
                transaction.reduce((total, item) => total + item.sellAmount * item.quantity, 0)
              )}
            </p>
          </div>
          <div className="mx-5 border-r w-52">
            ...
            <p className="font-light text-sm relative">
              <span className="font-light text-2xl">₹ {toThousands(handlePendingAmount())}</span>
            </p>
          </div>
        </div>
